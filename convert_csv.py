...
while copies_counter != copies+1:
	...
	for vid_id in video_list:
		for quality in quality_list:
			 f.write("tshark -nr ../../"+ pcap_path2 + "/" + vid_id + "/" + quality + "/" + vid_id +".cap -q -z io,stat,0.1,BYTES | grep -P "'"\d{1,2}\.{1}\d{1,2}\s+<>\s+\d{1,2}\.{1}\d{1,2}\s*\|\s+\d+"'"| awk -F '[ |]+' '{print $2"','"($5*8)}' > ../../" + pcap_path2 + "/" + vid_id + "/" + quality + "/" +vid_id+ ".csv\n")
